-- ============================================================================
-- FIX: Update debit_memo_items.received_price when credit memo is analyzed
-- ============================================================================
-- Problem: record_credit_memo_analysis() was only inserting into ndc_payment_history
-- but NOT updating the received_price in debit_memo_items table.
--
-- Solution: Modify the function to also update debit_memo_items.received_price
-- by matching NDC within the same debit memo.
-- ============================================================================
...
-- ============================================================================
-- VERIFICATION: After running this script, test by:
-- 1. Upload a credit memo PDF to a debit memo
-- 2. Check that debit_memo_items.received_price is populated:
--    SELECT dmi.id, dmi.ndc, dmi.product_name, dmi.ask_price, dmi.received_price
--    FROM debit_memo_items dmi
--    WHERE dmi.debit_memo_id = 'your-memo-id';
-- ============================================================================
