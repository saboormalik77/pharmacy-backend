-- Migration: fix_payment_tracking_grouped_by_return
-- Unpaid and paid memos grouped by return transaction with return-based pagination.
-- Safe to re-run.

-- ============================================================
-- UNPAID MEMOS GROUPED BY RETURN
-- ============================================================
CREATE OR REPLACE FUNCTION payment_list_unpaid_grouped_by_return(
          ...
          'memos',          COALESCE(
            jsonb_agg(
              DISTINCT jsonb_build_object(
                ...
                'daysOutstanding',   EXTRACT(DAY FROM (NOW() - COALESCE(dm.shipped_at, dm.created_at)))::int,
...
-- ============================================================
-- PAID MEMOS GROUPED BY RETURN
-- ============================================================
CREATE OR REPLACE FUNCTION payment_list_paid_grouped_by_return(
  p_destination  text    DEFAULT NULL,
  p_search       text    DEFAULT NULL,
  p_page         integer DEFAULT 1,
  p_limit        integer DEFAULT 10
