-- Function : warehouse_resolve_discrepancy
-- Arguments: p_discrepancy_id uuid, p_resolution text, p_resolution_notes text, p_resolved_by uuid
-- Type     : FUNCTION
-- =============================================================

DROP FUNCTION IF EXISTS public.warehouse_resolve_discrepancy(p_discrepancy_id uuid, p_resolution text, p_resolution_notes text, p_resolved_by uuid) CASCADE;

CREATE OR REPLACE FUNCTION public.warehouse_resolve_discrepancy(p_discrepancy_id uuid, p_resolution text, p_resolution_notes text DEFAULT NULL::text, p_resolved_by uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
