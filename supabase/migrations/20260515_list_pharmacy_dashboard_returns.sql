-- Pharmacy dashboard: only returns with debit memo ask prices + complete non-returnable reasons
-- Used by GET /api/dashboard/returns-list (dashboardService.getReturnsList)
...
COMMENT ON FUNCTION public.list_pharmacy_dashboard_returns(uuid) IS
  'Returns (as JSON array) eligible for pharmacy dashboard: debit memo items with ask_price, and no incomplete non-returnable reasons.';

GRANT EXECUTE ON FUNCTION public.list_pharmacy_dashboard_returns(uuid) TO authenticated, anon, service_role;
