...
export default function SetupAccountPage() {
  ...
  const verifyToken = async (t: string) => {
    try {
      const res = await fetch(`${API_BASE_URL}/auth/verify-invite`, {
        method: 'POST',
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/auth/complete-setup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Your password has been set and your account is now active. You can now log in to your pharmacy portal.
          </p>
          <button
            onClick={() => router.push('/login')}
            className="px-6 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-8 py-6 text-white">
          ...
          <p className="text-blue-100 mt-1">Set your password to get started with {inviteData?.pharmacyName}</p>
        </div>
