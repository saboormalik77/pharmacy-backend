...
-- Arguments: p_group_id uuid, p_outbound_tracking text, p_shipped_at timestamp with time zone, p_fedex_shipment_id text, p_fedex_labels jsonb
-- Type     : FUNCTION
-- =============================================================

DROP FUNCTION IF EXISTS public.ship_memo_group(p_group_id uuid, p_outbound_tracking text, p_shipped_at timestamp with time zone, p_fedex_shipment_id text, p_fedex_labels jsonb) CASCADE;

CREATE OR REPLACE FUNCTION public.ship_memo_group(p_group_id uuid, p_outbound_tracking text, p_shipped_at timestamp with time zone DEFAULT now(), p_fedex_shipment_id text DEFAULT NULL::text, p_fedex_labels jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
