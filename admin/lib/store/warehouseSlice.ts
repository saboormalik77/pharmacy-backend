...
export const addSurplus = createAsyncThunk<
    WarehouseSurplusItem,
    { transactionId: string; data: Record<string, any> },
    { rejectValue: string }
>('warehouse/addSurplus', async ({ transactionId, data }, { rejectWithValue }) => {
    try {
        const { apiClient } = await import('@/lib/api/apiClient');
        const res = await apiClient.post<{ status: string; data: WarehouseSurplusItem }>(
            `/admin/warehouse/${transactionId}/surplus`, data, true
        );
        return res.data;
    } catch (err: any) {
...
const warehouseSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // V2: resolveDiscrepancy
            .addCase(resolveDiscrepancy.pending, (state) => { state.isActionLoading = true; state.error = null; })
            .addCase(resolveDiscrepancy.fulfilled, (state) => { state.isActionLoading = false; })
