...
-- Arguments: p_group_by text, p_period text, p_page integer, p_limit integer
-- Type     : FUNCTION
-- =============================================================

DROP FUNCTION IF EXISTS public.payment_ask_vs_received(p_group_by text, p_period text, p_page integer, p_limit integer) CASCADE;

CREATE OR REPLACE FUNCTION public.payment_ask_vs_received(p_group_by text DEFAULT 'manufacturer'::text, p_period text DEFAULT NULL::text, p_page integer DEFAULT 1, p_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
