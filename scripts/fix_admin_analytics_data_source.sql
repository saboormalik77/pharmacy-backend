-- ============================================================
-- Fix Admin Analytics to use return_transactions data source
-- Problem: Analytics shows $0.00 because it uses return_reports table
-- Solution: Update to use return_transactions + return_transaction_items
--           (same data source as the fixed admin dashboard)
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- 1. KEY METRICS - ALL TIME TOTALS (using return_transactions)
  -- ============================================================
  
  -- Total Returns Value (sum from return_transactions aggregates)
  SELECT COALESCE(SUM(COALESCE(rt.total_returnable_value, 0) + COALESCE(rt.total_non_returnable_value, 0)), 0)
  ...
  -- Total Returns Count (count return_transactions)
  ...
  FROM return_transactions rt
  INNER JOIN pharmacy p ON p.id = rt.pharmacy_id
  WHERE rt.created_at::DATE >= v_current_month_start
  ...
  FROM pharmacy p
  WHERE p.created_at::DATE >= v_current_month_start
    AND (p_buying_group_id IS NULL OR p.created_by = p_buying_group_id);
  ...
  FROM return_transactions rt
  INNER JOIN pharmacy p ON p.id = rt.pharmacy_id
  WHERE rt.created_at::DATE >= v_last_month_start 
    AND rt.created_at::DATE < v_current_month_start
    AND (p_buying_group_id IS NULL OR p.created_by = p_buying_group_id);
  ...
  FROM pharmacy p
  WHERE p.created_at::DATE >= v_last_month_start 
    AND p.created_at::DATE < v_current_month_start
    AND (p_buying_group_id IS NULL OR p.created_by = p_buying_group_id);
  ...
  v_returns_value_change := CASE 
    ...
    WHEN v_current_month_value > 0 THEN 100.0
    ELSE 0
  END;
  ...
  v_total_returns_change := CASE 
    ...
    WHEN v_current_month_returns > 0 THEN 100.0
    ELSE 0
  END;
  ...
  v_pharmacies_change := CASE 
    ...
    WHEN v_current_month_pharmacies > 0 THEN 100.0
    ELSE 0
  END;

  -- ============================================================
  -- 5. RETURNS VALUE TREND (Past 12 months, using return_transactions)
  -- ============================================================
  ...
  monthly_data AS (
    ...
    FROM return_transactions rt
    INNER JOIN pharmacy p ON p.id = rt.pharmacy_id
    WHERE rt.created_at::DATE >= v_twelve_months_ago
      AND (p_buying_group_id IS NULL OR p.created_by = p_buying_group_id)
  ...
  -- ============================================================
  -- 6. TOP 5 PRODUCTS BY RETURNS VALUE (using return_transaction_items)
  -- ============================================================
  ...
  -- ============================================================
  -- 7. DISTRIBUTOR BREAKDOWN (using uploaded_documents for distributor mapping)
  -- ============================================================
  ...
  -- ============================================================
  -- 8. STATE BREAKDOWN (using return_transactions)
  -- ============================================================
  ...
  WITH state_stats AS (
    SELECT 
      ...
      COALESCE(SUM(COALESCE(rt.total_returnable_value, 0) + COALESCE(rt.total_non_returnable_value, 0)), 0) AS total_value
    FROM pharmacy p
    LEFT JOIN return_transactions rt ON rt.pharmacy_id = p.id
    WHERE (p_buying_group_id IS NULL OR p.created_by = p_buying_group_id)
    GROUP BY 
      CASE 
        WHEN p.physical_address IS NULL THEN 'Unknown'
...
COMMENT ON FUNCTION get_admin_analytics(UUID) IS
  'Get all analytics data for admin dashboard using return_transactions data source: key metrics, trends, top products, distributor and state breakdowns. '
