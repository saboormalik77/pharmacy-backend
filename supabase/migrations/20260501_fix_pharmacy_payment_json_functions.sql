-- ============================================================
-- Fix: Pharmacy Payment JSON Functions & Check Enhancement
-- Applies fcr_51_pharmacy_checks.sql that was never migrated.
-- This creates the missing _pharmacy_payment_to_json function
-- and all related check-payment infrastructure.
-- ============================================================
...
-- Unique constraint on check_number (when not null)
...
-- ────────────────────────────────────────────────────────────
-- 3. _pharmacy_payment_to_json helper (full version with check fields)
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION _pharmacy_payment_to_json(pharmacy_payments) TO anon, authenticated, service_role;


-- ────────────────────────────────────────────────────────────
-- 4. _get_manufacturer_credits helper
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION _get_manufacturer_credits(UUID) TO anon, authenticated, service_role;


-- ────────────────────────────────────────────────────────────
-- 5. RPC: pharmacy_payment_generate_check_number
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION pharmacy_payment_generate_check_number() TO anon, authenticated, service_role;


-- ────────────────────────────────────────────────────────────
-- 6. Data migration: populate new fields for existing payments
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- 7. RPC: pharmacy_payment_update (updated with check fields)
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION pharmacy_payment_update(UUID, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, TEXT, TIMESTAMPTZ, TEXT, TEXT) TO anon, authenticated, service_role;


-- ────────────────────────────────────────────────────────────
-- 8. RPC: pharmacy_payment_my_payments (with date range support)
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION pharmacy_payment_my_payments(UUID, TEXT, TEXT, DATE, DATE, INTEGER, INTEGER) TO anon, authenticated, service_role;


-- ────────────────────────────────────────────────────────────
-- 9. RPC: pharmacy_payment_check_pdf_data
-- ────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION pharmacy_payment_check_pdf_data(TEXT) TO anon, authenticated, service_role;
