-- Migration: fix_list_debit_memos_grouped_by_return
-- List debit memos grouped by their source return transaction.
-- Pagination is return-based (e.g., 10 returns per page), not memo-based.
--
-- Safe to re-run.

-- Helper: get the primary return transaction ID for a debit memo
-- (uses the first item's link to return_transaction_items)
CREATE OR REPLACE FUNCTION _get_debit_memo_return_id(p_memo_id UUID)
...
-- Main RPC: list debit memos grouped by return transaction
CREATE OR REPLACE FUNCTION list_debit_memos_grouped_by_return(
