-- ============================================================
-- FIX: Add reported_by column and update warehouse_verify_item_v2 function
-- ============================================================

-- Add the missing reported_by column to warehouse_discrepancies table
ALTER TABLE warehouse_discrepancies 
ADD COLUMN IF NOT EXISTS reported_by uuid;

-- Drop any existing foreign key constraint on reported_by (it may reference wrong table)
ALTER TABLE warehouse_discrepancies 
DROP CONSTRAINT IF EXISTS fk_warehouse_discrepancies_reported_by;

-- NOTE: We don't add a foreign key constraint because reported_by can be:
-- - A processor ID (from processors table)
-- - An admin ID (from admins table)
-- - A main admin ID (from main_admins table)
-- The column stores the user ID who reported, regardless of user type.

-- Add index for better performance
CREATE INDEX IF NOT EXISTS idx_warehouse_discrepancies_reported_by 
ON warehouse_discrepancies(reported_by);

COMMENT ON COLUMN warehouse_discrepancies.reported_by IS 'ID of the user (processor/admin/main_admin) who reported the discrepancy';

-- Update the warehouse_verify_item_v2 function to include missing parameters
DROP FUNCTION IF EXISTS public.warehouse_verify_item_v2(uuid, uuid, text, integer, text, uuid);

CREATE OR REPLACE FUNCTION public.warehouse_verify_item_v2(
  p_transaction_id uuid, 
  p_item_id uuid, 
  p_verification_status text, 
  p_actual_quantity integer DEFAULT NULL::integer, 
  p_condition_notes text DEFAULT NULL::text, 
  p_reported_by uuid DEFAULT NULL::uuid,
  p_non_returnable_reason text DEFAULT NULL::text,
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.warehouse_verify_item_v2(uuid, uuid, text, integer, text, uuid, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.warehouse_verify_item_v2(uuid, uuid, text, integer, text, uuid, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.warehouse_verify_item_v2(uuid, uuid, text, integer, text, uuid, text, text, text) TO service_role;

-- ============================================================
-- ✅ COMPLETED: Fixed warehouse_discrepancies and warehouse_verify_item_v2
-- ============================================================