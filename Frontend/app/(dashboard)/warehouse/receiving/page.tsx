      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between p-4 rounded-[4px] bg-[#f5f2f1] border-2 border-[#e2e2e2]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#000000]">Warehouse Receiving</h1>
              <p className="text-xs text-[#505454] mt-0.5">Receive, inspect, and classify returned packages</p>
            </div>
          </div>
        </div>
          ...
          <CardContent className="p-3">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search by Return ID or Shipment ID..." className="pl-7 h-7 text-xs border-[#e2e2e2] focus:border-[#e2e2e2]" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
        ...
        <div className="flex gap-2 border-b-2 border-[#e2e2e2] bg-white rounded-t-lg p-1">
          {[
            ...
            { label: 'Classified', value: 'classified', count: statusCounts.classified, color: 'bg-[#f5f2f1] text-[#516057] border-[#e2e2e2]' },
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {filteredRecords.length === 0 ? (
                    <tr><td colSpan={12} className="p-4 text-center text-[#6b7280] text-sm bg-[#f5f2f1]">No receiving records found</td></tr>
                  ) : (
                    filteredRecords.map((record, idx) => {
                      ...
                      return (
                        <tr key={record.id} className={`border-b ${idx % 2 === 0 ? 'bg-white' : 'bg-[#f5f2f1]/50'} hover:bg-[#f5f2f1] transition-colors`}>
                          <td className="p-2 font-semibold text-[#516057]">{record.id}</td>
                          <td className="p-2">
                            <Badge variant={getStatusVariant(record.status)} className={`text-xs border-2 ${getStatusColor(record.status)}`}>
                              {record.status.toUpperCase()}
                            </Badge>
                          </td>
                          <td className="p-2 font-mono text-[#505454]">{record.returnId}</td>
                          <td className="p-2 font-mono text-[#505454]">{record.shipmentId}</td>
                          <td className="p-2 text-[#505454]">{formatDate(record.receivedDate)}</td>
                          <td className="p-2 text-[#505454]">{record.receivedBy}</td>
                          <td className="p-2">
          ...
          <Card className="border-2 border-purple-200 bg-gradient-to-br from-white to-purple-50/30">
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-[4px] bg-purple-100">
                    <Package className="h-5 w-5 text-purple-600" />
                  </div>
                  <div>
                    <h3 className="font-bold text-base text-[#000000]">Receiving Record: {selectedRecord.id}</h3>
                    <p className="text-xs text-[#505454]">Return: {selectedRecord.returnId} • Shipment: {selectedRecord.shipmentId}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                <div className="p-2 rounded-[4px] border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-purple-100">
                ...
                <div className="p-2 rounded-[4px] border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100">
                  <div className="flex items-center gap-1 mb-1">
                ...
                <div className="p-2 rounded-[4px] border-2 border-indigo-200 bg-gradient-to-br from-indigo-50 to-indigo-100">
                  <div className="flex items-center gap-1 mb-1">
                ...
                <div className="p-2 rounded-[4px] border-2 border-green-200 bg-gradient-to-br from-green-50 to-green-100">
                  <div className="flex items-center gap-1 mb-1">
              ...
              {/* Packages Table */}
              <div className="mb-4">
                <h4 className="font-semibold text-sm mb-2 text-[#000000]">Packages ({selectedRecord.packages.length})</h4>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                    ...
                    <tbody>
                      {selectedRecord.packages.map((pkg, idx) => (
                        <tr key={pkg.id} className={`border-b ${idx % 2 === 0 ? 'bg-white' : 'bg-[#f5f2f1]/50'} hover:bg-purple-50`}>
              ...
              {/* Items Summary */}
              <div className="grid grid-cols-3 gap-2 p-3 rounded-[4px] bg-[#f5f2f1] border-2 border-green-200">
                <div>
