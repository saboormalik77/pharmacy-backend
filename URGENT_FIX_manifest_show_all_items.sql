-- ============================================================
-- URGENT FIX: Update get_manifest_data function to show ALL items
-- ============================================================
-- This fixes the issue where items with return_status = 'tbd' or null 
-- were not being displayed in the manifest
-- 
-- TO APPLY THIS FIX:
...
DECLARE
  ...
  v_all_items        jsonb;  -- NEW: Show all items regardless of status
...
BEGIN
  ...
  -- Processor name - FIXED: Use 'processors' table instead of 'admin_users'
  ...
  -- Returnable items (keep existing logic for backward compatibility)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'ndc',                rti.ndc,
      'ndc10',              rti.ndc_10,
  ...
  -- Non-returnable items (keep existing logic)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'ndc',                rti.ndc,
      'ndc10',              rti.ndc_10,
  ...
  RETURN jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'returnableItems',    COALESCE(v_returnable_items, '[]'::jsonb),
      'nonReturnableItems', COALESCE(v_non_returnable, '[]'::jsonb),
      'allItems',           v_all_items  -- NEW: Include all items for debugging
    )
  );
END;
$$;
...
COMMENT ON FUNCTION get_manifest_data(UUID) IS 
  'Get comprehensive manifest data for a return transaction including pharmacy, processor, and items with enhanced CII detection. Now includes allItems field for debugging.';

-- ============================================================
-- VERIFICATION QUERY (Optional - run after the function fix)
-- ============================================================
-- Test the function with the problematic transaction ID:
-- SELECT get_manifest_data('b69ef420-9363-4c62-b727-2f6b409df312');
-- ============================================================