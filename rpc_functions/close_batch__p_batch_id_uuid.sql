...
BEGIN
  ...
  -- Generate debit memos grouped by pharmacy + destination + labeler_id (NDC prefix)
  -- Grouping by labeler_id (first 5 chars of NDC) instead of free-text manufacturer name
  -- prevents duplicate memos when the same manufacturer has slight name variations
  -- (e.g. "Accord Healthcare Inc" vs "Accord Healthcare Inc." or case differences)
  FOR v_group IN
    SELECT
      rt.pharmacy_id,
      rti.destination,
