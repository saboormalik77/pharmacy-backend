-- FIX: add_return_transaction_item_with_validation calls
-- add_return_transaction_item(uuid, jsonb) but only add_return_transaction_item(jsonb) exists.
-- This patch re-creates the wrapper to inject transactionId into the jsonb payload
-- before calling the original single-argument function.
--
-- Run this in Supabase SQL Editor to fix the error:
--   "function add_return_transaction_item(uuid, jsonb) does not exist"
