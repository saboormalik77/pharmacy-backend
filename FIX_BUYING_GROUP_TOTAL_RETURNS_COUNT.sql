-- ============================================================
-- FIX: Buying Group Dashboard Total Returns Count
-- ============================================================
-- Total Returns should be the sum of:
-- 1. Returns created by pharmacies belonging to the buying group
-- 2. Returns created by processors for pharmacies in the buying group
-- ============================================================
-- NOTE: The pharmacy table uses "created_by" column to link to buying groups,
--       NOT "buying_group_id". This was causing the column not found error.
-- ============================================================

-- Update the get_admin_dashboard_stats function to correctly count total returns
CREATE OR REPLACE FUNCTION public.get_admin_dashboard_stats(
  p_pharmacy_id uuid DEFAULT NULL::uuid,
...
DECLARE
    ...
    -- Additional variables for total returns
    v_total_returns                 INTEGER;
    v_total_returns_this_month      INTEGER;
    v_total_returns_last_month      INTEGER;
...
BEGIN
    ...
    -- ============================================================
    -- STAT 1: Total Pharmacies (scoped to buying group via created_by)
    -- ============================================================
    ...
    -- ============================================================
    -- STAT 3: Returns Value (scoped to buying group via pharmacy.created_by)
    -- ============================================================

    SELECT COALESCE(SUM(ud.total_credit_amount), 0)::NUMERIC
    ...
    -- ============================================================
    -- STAT 4: Total Returns Count (scoped to buying group via pharmacy.created_by)
    -- This is the SUM of:
    --   - Returns created by pharmacies themselves (processor_id IS NULL)
    --   - Returns created by processors for these pharmacies (processor_id IS NOT NULL)
    -- ============================================================

    IF p_buying_group_id IS NOT NULL THEN
        -- Total returns for pharmacies in this buying group
        SELECT COUNT(*)::INTEGER
        INTO v_total_returns
        FROM return_transactions rt
        JOIN pharmacy p ON rt.pharmacy_id = p.id
        WHERE p.created_by = p_buying_group_id;

        -- This month
        SELECT COUNT(*)::INTEGER
        INTO v_total_returns_this_month
        FROM return_transactions rt
        JOIN pharmacy p ON rt.pharmacy_id = p.id
        WHERE p.created_by = p_buying_group_id
        ...
        -- Last month
        SELECT COUNT(*)::INTEGER
        INTO v_total_returns_last_month
        FROM return_transactions rt
        JOIN pharmacy p ON rt.pharmacy_id = p.id
        WHERE p.created_by = p_buying_group_id
          AND rt.created_at >= v_last_month_start
          AND rt.created_at <  v_current_month_start;
    ELSE
        -- No buying group filter - count all returns
        SELECT COUNT(*)::INTEGER INTO v_total_returns FROM return_transactions;
    ...
    IF v_total_returns_last_month > 0 THEN
        v_total_returns_change := ROUND(((v_total_returns_this_month - v_total_returns_last_month)::NUMERIC / v_total_returns_last_month * 100)::NUMERIC, 1);
    ELSE
        v_total_returns_change := CASE WHEN v_total_returns_this_month > 0 THEN 100.0 ELSE 0.0 END;
    END IF;
    ...
    IF p_period_type = 'yearly' THEN
        ...
        earnings AS (
            SELECT
                EXTRACT(YEAR FROM ud.report_date)::INTEGER        AS year,
                ROUND(SUM(ud.total_credit_amount)::NUMERIC, 2)    AS value,
                COUNT(*)::INTEGER                                  AS documents_count
            FROM uploaded_documents ud
            JOIN pharmacy p ON p.id = ud.pharmacy_id
            WHERE ud.report_date >= v_start_date
              AND ud.report_date <= v_end_date
              AND ud.total_credit_amount IS NOT NULL
              AND (p_pharmacy_id     IS NULL OR ud.pharmacy_id  = p_pharmacy_id)
              AND (p_buying_group_id IS NULL OR p.created_by    = p_buying_group_id)
            GROUP BY EXTRACT(YEAR FROM ud.report_date)
        )
        SELECT COALESCE(jsonb_agg(
            jsonb_build_object(
                'period',         y.year::TEXT,
    ...
    ELSE
        ...
        earnings AS (
            SELECT
                TO_CHAR(ud.report_date, 'YYYY-MM')         AS period,
                ROUND(SUM(ud.total_credit_amount)::NUMERIC, 2) AS value,
                COUNT(*)::INTEGER                              AS documents_count
            FROM uploaded_documents ud
            JOIN pharmacy p ON p.id = ud.pharmacy_id
            WHERE ud.report_date >= v_start_date
              ...
              AND (p_pharmacy_id     IS NULL OR ud.pharmacy_id  = p_pharmacy_id)
              AND (p_buying_group_id IS NULL OR p.created_by    = p_buying_group_id)
            GROUP BY TO_CHAR(ud.report_date, 'YYYY-MM')
        )
        SELECT COALESCE(jsonb_agg(
            jsonb_build_object(
                'period',         m.period,
    ...
    -- ============================================================
    -- Build final result (matching expected structure from backend service)
    -- ============================================================
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.get_admin_dashboard_stats(uuid, text, integer, uuid) TO anon;
...
-- ============================================================
-- ✅ COMPLETED: Fixed buying group total returns count
-- - Uses "created_by" column (not "buying_group_id") to filter pharmacies
-- - Total Returns = All returns for pharmacies where created_by = buying_group_id
-- - Returns proper nested structure expected by backend service:
--   stats.totalPharmacies, stats.activeDistributors, stats.returnsValue, stats.totalReturns
-- ============================================================
