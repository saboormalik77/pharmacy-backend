...
export default function ReturnsPage() {
    ...
    const fetchTransactions = useCallback(async () => {
        ...
        try {
            ...
            const res = await apiClient.get<any>('/return-transactions', params, true);
            ...
            setError(err.message || 'Failed to load return transactions');
            setTransactions([]);
            setPagination(null);
        } finally {
            setIsLoading(false);
    ...
    const handleStatusAction = async () => {
        ...
        try {
            const res = await apiClient.post(`/return-transactions/${tx.id}/${action}`, {}, true);
            if (res.status === 'success') {
                ...
                throw new Error(res.message || `Failed to ${action} return`);
            }
        } catch (err: any) {
            showToast(err.message || `Failed to ${action} return`, 'error');
            setActionModal(null);
        } finally {
    ...
    const handleUpdate = async () => {
        ...
        setIsActionLoading(true);
        try {
            const res = await apiClient.patch(`/return-transactions/${editModal.id}`, editForm, true);
            if (res.status === 'success') {
    ...
    const handleDelete = async () => {
        ...
        setIsActionLoading(true);
        try {
            const res = await apiClient.delete(`/return-transactions/${deleteModal.id}`, true);
            if (res.status === 'success') {
    ...
    return (
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => router.push('/returns/create')} className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-xs font-medium bg-primary-600 text-white hover:bg-primary-700 transition-colors">
            ...
            <div className="bg-white rounded-lg shadow">
                ...
                <div className="flex flex-wrap gap-2 px-3 py-2 border-b border-gray-100">
                    ...
                    <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setCurrentPage(1); }} className="px-2.5 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-primary-500" title="Date from" />
                    <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setCurrentPage(1); }} className="px-2.5 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-primary-500" title="Date to" />
                </div>
                                            ...
                                            <td className="px-3 py-1.5" onClick={e => e.stopPropagation()}>
                                                <div className="flex items-center justify-end gap-0.5">
                                                    ...
                                                    {canDoAction(tx, 'finalize') && (
                                                        <button onClick={() => router.push(`/returns/${tx.id}`)} className="p-1 text-gray-400 hover:text-purple-600 hover:bg-purple-50 rounded" title="Finalize">
